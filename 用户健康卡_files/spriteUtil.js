define(function(require) {
    ...
    function getVueRoute() {
        ...
        // The logged on account have no permission to access the page.
        if ( indexPages.length == 0 && routes.length == 0 ) {
        	var noPermissionViewPage = {
        			name 		: 'nopermission',
        			path 		: '/nopermission',
        			component 	: spriteUtils.loadComponent( 'nopermission' ),
        			meta		: {
        				keepAlive : true
        			}
        	};
        	indexPages.push ( noPermissionViewPage );
        }
...
//                }, config);
    ...
    function getWechatSign() {
        ...
        // if (/micromessenger/.test(navigator.userAgent.toLowerCase())) {
        	...
        	url = WIS_CONFIG.ROOT_PATH + '/sys/itpub/MobileCommon/dingtalk/config.do';
        	require(['BH_MOBILE'], function(BH_MOBILE) {
                window.BH_MOBILE = BH_MOBILE;
                //请求当前页面的微信签名
                MOB_UTIL.doPost({
